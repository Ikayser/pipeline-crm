...
export default function CRMDashboard() {
  ...
  // Load prospects when session is available
  useEffect(() => {
    if (session) {
      loadProspects();
    }
  ...
  const loadProspects = async () => {
    ...
    // Load engagements for all prospects
    ...
    // Merge engagements into prospects
    ...
    if (prospect.id) {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          last_engagement: prospect.last_engagement,
  ...
  const handleUpdateStage = async (prospect, newStage) => {
    ...
    await loadProspects();
    setSelectedProspect({ ...prospect, stage: newStage });
    setSyncing(false);
  };
  ...
  const handleAddEngagement = async (prospectId, engagement) => {
    ...
    if (engagementError) {
      console.error('Error adding engagement:', engagementError);
    }

    // Update last_engagement on prospect
    const { error: prospectError } = await supabase
      .from('prospects')
      .update({ last_engagement: engagement.date })
      .eq('id', prospectId);

    if (prospectError) {
      console.error('Error updating last engagement:', prospectError);
    }

    await loadProspects();
    setShowEngagementForm(false);
    
    // Refresh selected prospect
    const updated = prospects.find(p => p.id === prospectId);
    if (updated) {
      setSelectedProspect({ ...updated, last_engagement: engagement.date });
    }
  ...
  const handleImportCSV = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      const newProspects = lines.slice(1).filter(line => line.trim()).map((line) => {
  ...
  const handleExportCSV = () => {
    ...
    const rows = prospects.map(p => [
      p.company, p.contact, p.title || '', p.linkedin || '', p.work_type, p.budget, p.stage, p.last_engagement, p.context
    ]);
    ...
    return (
      <div style={styles.authContainer}>
        <div style={styles.authBox}>
          ...
          <p style={styles.authText}>Loading...</p>
        </div>
      </div>
    );
  }
                  ...
                  <div style={styles.pipelineCards}>
                    {prospects.filter(p => p.stage === stage).map(prospect => (
                      <div
                        ...
                        style={{
                          ...styles.prospectCard,
                          ...(daysSince(prospect.last_engagement) > 7 ? styles.prospectCardStale : {})
                        }}
                      ...
                      >
                        ...
                        <div style={styles.cardMeta}>
                          <span>{prospect.work_type}</span>
                          <span>{formatCurrency(prospect.budget || 0)}</span>
                        </div>
                        <div style={styles.cardEngagement}>
                          {daysSince(prospect.last_engagement)}d ago
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <tbody>
                {prospects.map(prospect => (
                  ...
                  >
                    ...
                    <td style={styles.td}>{prospect.work_type}</td>
                    <td style={styles.td}>{formatCurrency(prospect.budget || 0)}</td>
                    <td style={styles.td}>{prospect.stage}</td>
                    <td style={{
                      ...styles.td,
                      ...(daysSince(prospect.last_engagement) > 7 ? { fontWeight: 700 } : {})
                    }}>
                      {formatDate(prospect.last_engagement)} ({daysSince(prospect.last_engagement)}d)
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div style={styles.insightsGrid}>
              ...
              <div style={styles.insightCard}>
                ...
                <div style={styles.typeList}>
                  {WORK_TYPES.map(type => {
                    const count = prospects.filter(p => p.work_type === type).length;
                    const value = prospects.filter(p => p.work_type === type).reduce((sum, p) => sum + (p.budget || 0), 0);
            ...
            <div style={styles.sidebarContent}>
              ...
              <div style={styles.detailRow}>
                <div style={styles.detailSection}>
                  ...
                  <p style={styles.detailValue}>{selectedProspect.work_type}</p>
                </div>
                <div style={styles.detailSection}>
                  <label style={styles.detailLabel}>Budget</label>
                  <p style={styles.detailValue}>{formatCurrency(selectedProspect.budget || 0)}</p>
                </div>
              </div>
              ...
              <div style={styles.detailSection}>
                ...
                <div style={styles.stageSelector}>
                  {STAGES.map(stage => (
                    <button
                      key={stage}
                      onClick={() => handleUpdateStage(selectedProspect, stage)}
                      style={{
                        ...styles.stageButton,
              ...
              <div style={styles.detailSection}>
                ...
                {(!selectedProspect.engagements || selectedProspect.engagements.length === 0) ? (
    ...
    <div style={styles.authContainer}>
      <div style={styles.authBox}>
        ...
        <form onSubmit={handleSubmit} style={styles.authForm}>
          ...
          <button type="submit" style={styles.actionButtonPrimary} disabled={loading}>
...
function ProspectForm({ prospect, onSave, onCancel }) {
  const [form, setForm] = useState(prospect ? {
    ...prospect,
    work_type: prospect.work_type || 'Consulting',
    last_engagement: prospect.last_engagement || new Date().toISOString().split('T')[0]
  } : {
    company: '',
    contact: '',
    linkedin: '',
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.formRow}>
            ...
            <div style={styles.formGroup}>
              ...
              <input
                type="date"
                value={form.last_engagement}
                onChange={e => setForm({ ...form, last_engagement: e.target.value })}
                style={styles.input}
              />
            </div>
          </div>
